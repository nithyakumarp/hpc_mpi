...
int main(int argc, char* argv[])
{
	if(argc != 3)
		...
		long int i=0, j=0, sum_check=0;
		for(i=0;i<num;i++){
			hist_global[i] = 0;
			hist_node[i] = 0;
		}

		int num_node = num / nodes;
		...
		MPI_Scatter(data_global, num_node, MPI_INT, data_node, num_node, MPI_INT, 0, MPI_COMM_WORLD);
		...
		//Histogram - all nodes have all classes but divided data
		for(i=0;i<class;i++){
			...
			for(j=0;j<num_node;j++){
				if(data_node[j] > lower_limit && data_node[j]<=upper_limit){
					//printf("%d ",data_node[j]);
					hist_node[i]++;
				}
			}
			//printf("\n");
		...
		MPI_Reduce(&hist_node, &hist_global, class, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
